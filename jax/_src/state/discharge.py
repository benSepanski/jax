...
def transform_array(x, transforms):
  ...
  for transform in transforms:
    ...
    match transform:
      ...
      case _:
        if hasattr(transform, "transform_array"):
...
def transform_swap_array(x, transforms, val):
  ...
  for intermediate, transform in reversed(zip(intermediates[:-1], transforms)):
    ...
    elif hasattr(transform, "transform_array"):
      new_x = transform.transform_array(new_x)
    else:
      raise NotImplementedError(f"Unsupported transform: {transform}")
...
def _run_state_jvp(primals: Sequence[Any], tangents: Sequence[Any], *,
  ...
  closed_jvp_jaxpr, _ = ad.jvp_jaxpr(jaxpr, nonzero_tangents, [])
  jvp_jaxpr_, jvp_consts = closed_jvp_jaxpr, closed_jvp_jaxpr.consts
