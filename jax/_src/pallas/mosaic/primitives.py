...
AccessAssumption = tpu_core.AccessAssumption
AccessAssumptionTransform = tpu_core.AccessAssumptionTransform
...
def annotate(
  ...
  * ``no_hazard``: (VMEM-only) Sets the read-after-write (RAW) hazard latency
    to 1 cycle while keeping data dependencies. Always sound to add on
    supported hardware, but hardware delays may hurt performance if legitimate
    RAW hazards exist.
  * ``no_hazard_no_deps``: (VMEM-only) Drops all memory dependencies between
  ...
  ``no_hazard``, or ``no_hazard_no_deps``. Hazard overrides (``no_hazard`` and
  ``no_hazard_no_deps``) apply exclusively to VMEM allocations.
  ...
  assert isinstance(underlying_aval, state.AbstractRef), (
      f"Expected AbstractRef, got {type(underlying_aval)}"
  )
  ...
  for transform in ref.transforms:
    if isinstance(transform, AccessAssumptionTransform):
      assumption = transform.assumption.union(assumption)
    else:
      new_transforms.append(transform)
  return state_types.TransformedRef(
      ref.ref, (*new_transforms, AccessAssumptionTransform(assumption))
  )
